...
def keypress(e):
    ...
    print(e.keycode)
    if e.keycode == 38 and empty > cellnumber - 1: # up for mac 8320768 
        field[empty]['text'], field[empty-cellnumber]['text'] = field[empty-cellnumber]['text'], field[empty]['text']
    if e.keycode == 40 and empty < n - cellnumber: # down for mac 8255233
        field[empty]['text'], field[empty+cellnumber]['text'] = field[empty+cellnumber]['text'], field[empty]['text']
    if e.keycode == 39 and empty % cellnumber != 0: # right for mac 8189699
            field[empty]['text'], field[empty-1]['text'] = field[empty-1]['text'], field[empty]['text']
    if e.keycode == 37 and empty not in [ i  for i in range(cellnumber - 1, n, cellnumber)]:# left for mac 8124162 #% 4 != cellnumber - 1:
